...
class LocalBuffer(object):
    ...
    def push(self, state, next_state, action, reward, mask, rnn_state, target_rnn_state):
        ...
        if (len(self.local_memory) + len(self.over_lapping_from_prev)) == sequence_length or mask == 0:
            ...
            while len(self.local_memory) < sequence_length:  # zero padding to standardize length of the each experience
                self.local_memory.append(Transition(torch.tensor([.0, .0, .0, .0], dtype=torch.float).to(self.device),
                                                    torch.tensor([.0, .0, .0, .0], dtype=torch.float).to(self.device), 0, 0, 0,
                                                    torch.zeros([2, 1, cell_size]).view(2, -1).to(self.device), torch.zeros([2, 1, cell_size]).view(2, -1).to(self.device)))  # rnn state = [(hidden,cell), seq_, dim]
    ...
    def get(self):
        ...
        for episode, length in episodes:
            ...
            batch_state.append(torch.stack(list(batch.state)).to(self.device))
            batch_next_state.append(torch.stack(list(batch.next_state)).to(self.device))
            batch_action.append(torch.tensor(list(batch.action)).to(self.device))
            batch_reward.append(torch.tensor(list(batch.reward)).to(self.device))
            batch_mask.append(torch.tensor(list(batch.mask)).to(self.device))
            batch_rnn_state.append(torch.stack(list(batch.rnn_state)).to(self.device))
            batch_target_rnn_state.append(torch.stack(list(batch.target_rnn_state)).to(self.device))
...
class Memory(object):
    ...
    def sample(self, batch_size):
        ...
        for episode in episodes:
            batch_state.append(episode.state.clone())
            batch_next_state.append(episode.next_state.clone())
            batch_action.append(episode.action.clone())
            batch_reward.append(episode.reward.clone())
            batch_mask.append(episode.mask.clone())
            batch_rnn_state.append(episode.rnn_state.clone())
            batch_target_rnn_state.append(episode.target_rnn_state.clone())
...
class Learner:
    ...
    def train(self, batch, lengths):
        ...
        batch_size = torch.stack(batch.state).contiguous().size()[0]
        states = torch.stack(batch.state).contiguous().view(batch_size, sequence_length, self.q.num_inputs)
        next_states = torch.stack(batch.next_state).contiguous().view(batch_size, sequence_length, self.q.num_inputs)
        actions = torch.stack(batch.action).contiguous().view(batch_size, sequence_length, -1).long()
        rewards = torch.stack(batch.reward).contiguous().view(batch_size, sequence_length, -1)
        masks = torch.stack(batch.mask).contiguous().view(batch_size, sequence_length, -1)
        rnn_state = torch.stack(batch.rnn_state).contiguous().view(batch_size, sequence_length, 2, -1)
        target_rnn_state = torch.stack(batch.target_rnn_state).contiguous().view(batch_size, sequence_length, 2, -1)
...
def actor_process(actor_id, n_actors, model, target_model, exp_q, lock, device):
    actor = Actor(actor_id, n_actors, model, target_model, exp_q, lock, device)
    actor.run()


class Actor:
    def __init__(self, actor_id, n_actors, model, target_model, share_exp_mem, lock, device):
        self.device = device
        self.env = gym.make('CartPole-v1', render_mode = 'rgb_array')
        self.actor_id = actor_id
        self.epsilon = 0.1 + (actor_id / 7) / n_actors  # 0.4 ** (1 + actor_id * 7 / (n_actors - 1))

        self.local_buffer = LocalBuffer(device)
        self.q = model
        self.q_target = target_model
...
def main():
    ...
    for actor_id in range(n_actors):
        processes.append(mp.Process(
            ...
            args=(actor_id, n_actors, model, target_model, experience_memory, lock, device)))
